  ...
  if (error) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4">
        <p className="text-sm text-red-700">Error loading questions: {error.message}</p>
      </div>
    );
  }
    ...
    <div>
      ...
      {items && items.length > 0 ? (
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item: Record<string, unknown>) => (
                <tr key={item.id as string} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{String(item.question_text ?? "—")}</td>
                  <td className="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{String(item.question_type ?? "—")}</td>
                  ...
                  <td className="px-6 py-4 text-right text-sm whitespace-nowrap">
                    ...
                    <DeleteButton id={item.id as string} table="questions" />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="card text-center py-12">
          ...
          <h3 className="mt-4 text-sm font-semibold text-gray-900">No questions</h3>
          <p className="mt-1 text-sm text-gray-500">Get started by adding your first question.</p>
          <Link href="/dashboard/questions/new" className="btn-primary mt-6 inline-flex">
            Add Question
          </Link>
        </div>
      )}
