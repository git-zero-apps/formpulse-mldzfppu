  ...
  if (error) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4">
        <p className="text-sm text-red-700">Error loading surveys: {error.message}</p>
      </div>
    );
  }
    ...
    <div>
      ...
      {items && items.length > 0 ? (
              ...
              {items.map((item: Record<string, unknown>) => (
                <tr key={item.id as string} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right text-sm whitespace-nowrap">
                    <Link href={`/dashboard/surveys/${item.id}/edit`} className="text-brand-600 hover:text-brand-800 font-medium mr-4">
                      Edit
                    </Link>
                    <DeleteButton id={item.id as string} table="surveys" />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="card text-center py-12">
          ...
          <h3 className="mt-4 text-sm font-semibold text-gray-900">No surveys</h3>
          <p className="mt-1 text-sm text-gray-500">Get started by adding your first survey.</p>
          <Link href="/dashboard/surveys/new" className="btn-primary mt-6 inline-flex">
            Add Survey
          </Link>
        </div>
      )}
